...
def KDE(
    ...
    stacked = az.extract_dataset(post)
    
    X = t.tensor(stacked.variable.values)
    ...
    def f_hat(X):
        ...
        phi = lambda z: t.reciprocal(t.sqrt(t.tensor(2*np.pi))) * t.exp(-1/2 * (z**2))

        return (1/(n*h)) * sum([phi(((x-X[i])/h)) for i in range(0,n)])
